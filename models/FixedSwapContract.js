const {fixedswap} = require('../interfaces')
...
module.exports = class FixedSwapContract {
  ...
  async __sendTx(f, call = false, value, handler) {
    var res
    if (this.acc && !call) {
      let data = f.encodeABI()
      res = await this.params.contract.send(this.acc.getAccount(), data, value, handler)
  ...
  async __deploy(params, callback) {
    return await this.params.contract.deploy(this.acc, this.params.contract.getABI(), this.params.contract.getJSON().bytecode, params, callback)
  }
  ...
  async withdrawableUnsoldTokens() {
    var res = 0
    if ((await this.hasFinalized()) && !(await this.wereUnsoldTokensReedemed())) {
  ...
  async getAddressPurchaseIds({address}) {
    let res = await this.__sendTx(this.params.contract.getContract().methods.getMyPurchases(address), true)
    return res.map((id) => Numbers.fromHex(id))
  }
  ...
  async swap({tokenAmount, handler}) {
    let amountWithDecimals = Numbers.toSmartContractDecimals(tokenAmount, this.getDecimals())
    let ETHCost = await this.getETHCostFromTokens({
      tokenAmount,
    })
    let ETHToWei = Numbers.toSmartContractDecimals(ETHCost, 18)
    return await this.__sendTx(this.params.contract.getContract().methods.swap(amountWithDecimals), false, ETHToWei, handler)
  }
  ...
  async asyncredeemTokens({purchase_id}) {
    return await this.__sendTx(this.params.contract.getContract().methods.redeemTokens(purchase_id))
  }
  ...
  async redeemGivenMinimumGoalNotAchieved({purchase_id}) {
    return await this.__sendTx(this.params.contract.getContract().methods.redeemGivenMinimumGoalNotAchieved(purchase_id))
  }
  ...
  async withdrawUnsoldTokens() {
    return await this.__sendTx(this.params.contract.getContract().methods.withdrawUnsoldTokens())
  }
   ...
   */
  async withdrawFunds() {
    return await this.__sendTx(this.params.contract.getContract().methods.withdrawFunds())
  }
   ...
   */
  async approveFundERC20({tokenAmount, callback}) {
   ...
   * @returns {Boolean}
   */
  async isApproved({tokenAmount, address}) {
  ...
  async fund({tokenAmount, callback}) {
    ...
    return await this.__sendTx(this.params.contract.getContract().methods.fund(amountWithDecimals), null, null, callback)
  }
  ...
  async addWhitelistedAddress({addresses}) {
    ...
    return await this.__sendTx(this.params.contract.getContract().methods.add(addresses))
  }
   ...
   */
  async removeWhitelistedAddress({address, index}) {
    return await this.__sendTx(this.params.contract.getContract().methods.remove(address, index))
  }
   ...
   */
  async safePull() {
    return await this.__sendTx(this.params.contract.getContract().methods.safePull(), null, 0)
  }
  ...
  async removeOtherERC20Tokens({tokenAddress, toAddress}) {
    return await this.__sendTx(this.params.contract.getContract().methods.removeOtherERC20Tokens(tokenAddress, toAddress))
  }
